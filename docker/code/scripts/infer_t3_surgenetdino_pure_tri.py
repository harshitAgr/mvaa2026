#!/usr/bin/env python3
"""Infer T3 with the equal probability mean of three surgical-DINOv2 models."""
...
def main() -> int:
    ...
    print("Fusion: mean(C_979A,C_675A,C_ALL6); native soft probability fusion; A_DR absent")
    ...
    for index, path in enumerate(files, 1):
        ...
        mask = (native_c > args.threshold)[0, 0].to(torch.uint8).cpu().numpy() * 255
        if device.type == "cuda":
            torch.cuda.synchronize()
